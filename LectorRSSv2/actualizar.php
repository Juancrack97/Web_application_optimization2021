...
$resultado = $conexion->query("SELECT link FROM feeds");
...
if ($resultado -> num_rows > 0) {
    ...
    for ($i = 0; $i < $itemQty; $i++) {
        ...
        $SelectEnt = $conexion->query("SELECT Count('Titulo') as total FROM entradas where Link='$Link'");
        while($row = $SelectEnt->fetch_assoc())
        {
            $rowcount=$row['total'];
        } 
